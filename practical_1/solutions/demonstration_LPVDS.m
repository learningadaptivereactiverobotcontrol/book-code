...
% --------------- Study dataset to improve it ---------------

% Check each trajectory position for overlaps
% for i = 1:nTraj
%     hold on;
%     disp(i)
%     plot3(trajectories(1,:,i), trajectories(2,:,i), trajectories(3,:,i))
%     pause()
% end

% Plot X, Y, Z components of velocity to remove beggining and end phases (0
% velocity phases), and assess noise  
% for i = 1:nTraj
%     hold on;
%     subplot(3,2,1); 
%     plot(trajectories(1,:,i))
%     
%     hold on;
%     subplot(3,2,2); 
%     plot(trajectories(2,:,i))
%     
%     hold on;
%     subplot(3,2,3); 
%     plot(trajectories(3,:,i))
%     
%     hold on;
%     subplot(3,2,4); 
%     plot(trajectories(4,:,i))
%     
%     hold on;
%     subplot(3,2,5); 
%     plot(trajectories(5,:,i))
%     
%     hold on;
%     subplot(3,2,6); 
%     plot(trajectories(6,:,i))
% end

% Post-processing

% Select non-overlapping trajectories
selectedTrajectories = [1, 3, 4, 5, 7, 9, 11, 12];
%selectedTrajectories = 1:13;

% Filter out noise
order = 3;
framelen = 15;
for i = 1:nTraj
    trajectories(:,:,i) = sgolayfilt(trajectories(:,:,i)', order, framelen)';
end

% Select relevant data window
minIndex = 36;
maxIndex = 170;

nTraj = length(selectedTrajectories);
nPoints = maxIndex - minIndex + 1;
...
% Parse dataset
for i = 1:nTraj
    Data(:,(i-1)*nPoints+1:i*nPoints) = trajectories(:,minIndex:maxIndex,selectedTrajectories(i));
    x0_all(:,i) = trajectories(1:3,1,selectedTrajectories(i));
    attractor = attractor + trajectories(1:3,end,selectedTrajectories(i));
end
