...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Impedance_controller_minimal2(robot,Impedance_Damping,Impedance_Stifness,External_force)
clc
...
desired.Stifness=Impedance_Stifness;
...
while (counter*dt<Time)
    ...
    Tau(:,counter)=update_control_law(robot,desired);
...
figure2.Position = [0.005  * screensize(3), 0.1  * screensize(4), 0.6 * screensize(3), 0.8 * screensize(4)];  
axes2=subplot(3,1,1,'Parent',figure2);
title('Impedance controller without inertia compensation', 'FontSize',12)
hold(axes2,'on');
box(axes2,'on');
grid(axes2,'on');
...
set(legend2,'Interpreter','latex');
%% Ploting torque
axes2 = subplot(3,1,2,'Parent',figure2);
hold(axes2,'on');
box(axes2,'on');
grid(axes2,'on');
set(axes2,'TickLabelInterpreter','latex');
...
axes2 = subplot(3,1,3,'Parent',figure2);
hold(axes2,'on');
box(axes2,'on');
grid(axes2,'on');
set(axes2,'TickLabelInterpreter','latex');
...
function Tau=update_control_law(robot,desired)

Tau=robot.Collios*robot.Dq+robot.Gravity+robot.Inertia*desired.DDq...
    -(desired.Damping*(robot.Dq)...
    +desired.Stifness*(robot.q-desired.q));
