...
%%%  Example: 2-DOF Robotic arm- impedance controller- Approach two
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Impedance_controller_approch_two(robot,Impedance_Inertia,Impedance_Damping,Impedance_Stifness,Impedance_mu,Impedance_muQ,External_force)
clc
...
desired.mu1_DQ=Impedance_muQ{1,1};
desired.mu2_DQ=Impedance_muQ{1,2};
...
while (counter*dt<Time)
    ...
    desired=Update_the_stiffness_damping(robot,desired,2*time(counter));
...
figure1.Position =  [0.335 * screensize(3), 0.15 * screensize(4), 0.33 * screensize(3), 0.8 * screensize(4)];  
axes1=subplot(3,1,1,'Parent',figure1);
title('Robot parameters, Approach 2','Fontsize',12);
hold(axes1,'on');
box(axes1,'on');
grid(axes1,'on');
...
figure2 = figure;
screensize = get(groot, 'Screensize'); 
figure2.Position = [0.335  * screensize(3), 0.02  * screensize(4), 0.33 * screensize(3), 0.35 * screensize(4)];  
axes1=subplot(2,1,1,'Parent',figure2);
title('Impedance variables, Approach 2','Fontsize',12);
hold(axes1,'on');
box(axes1,'on');
grid(axes1,'on');
set(axes1 ,'TickLabelInterpreter','latex');
...
function desired=Update_the_stiffness_damping(robot,desired,time)
% Update the stifness and the damping matrices
Theta1=((robot.q-desired.mu1)'*(robot.q-desired.mu1)+   ((robot.Dq-desired.mu1_DQ)'*((robot.Dq-desired.mu1_DQ))))*time;
Theta2=((robot.q-desired.mu2)'*(robot.q-desired.mu2)+((robot.Dq-desired.mu2_DQ)'*((robot.Dq-desired.mu2_DQ))));
sum=Theta1+Theta2;
Theta1=Theta1/sum;
Theta2=Theta2/sum;
