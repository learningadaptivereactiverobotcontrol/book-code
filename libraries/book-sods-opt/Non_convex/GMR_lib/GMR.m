function [y, beta,Numerator,Denominator,A] = GMR(Priors, Mu, Sigma, x, in, out)
%
...
Numerator=zeros(size(x,2),nbStates);
Pxi=zeros(size(x,2),nbStates);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:nbStates
    Numerator(:,i)=gaussPDF(x, Mu(in,i), Sigma(in,in,i));
    Pxi(:,i) = Priors(i).*Numerator(:,i);
end
Denominator=sum(Pxi,2)+realmin;
beta = Pxi./repmat(Denominator,1,nbStates);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j=1:nbStates
    A(:,:,j)=Sigma(out,in,j)/(Sigma(in,in,j));
  y_tmp(:,:,j) =  A(:,:,j)* (x);
%   y_tmp(:,:,j) =  (x);
end
